...
class KernelAttention(tf.keras.layers.MultiHeadAttention):
  ...
  def call(self, query, value, key=None, attention_mask=None, training=False):
    ...
    if self._begin_kernel > 0:
      attention_output_softmax, attention_scores = self._compute_attention(
      ...
      attention_output_kernel, attention_scores = self._compute_attention(
      ...
      attention_output, attention_scores = self._compute_attention(query, key, value,
